...
#define SAVE_DATA_FILE  "/data/data.txt"
#define BUF_SIZE			512
#define ENABLE_SAVE_FILE
...
read_printer_data()
{
	...
	while (1) {
		...
		if (retval && (fd[0].revents & POLLRDNORM)) {
			...
			if (bytes_read < 0) {
				printf("Error %d reading from %s,%ld\n",
						fd[0].fd, PRINTER_FILE,actual);
				close(fd[0].fd);
				fclose(fp);
				return(-1);
			} else if (bytes_read > 0) {
				#ifndef ENABLE_SAVE_FILE
				...
				if(fwrite(buf, 1, bytes_read, fp)!=bytes_read)
					{
						printf("Error writing to %s:%ld,%d\n",SAVE_DATA_FILE,actual,bytes_read);	
						break;			
					}
					actual += bytes_read;
				#endif
			...
			printf("1.receved data size:%ld\n",actual);

		}
		if(actual>0)
			check_time++;
		if(check_time>10) //wait for 10 seconds to sync print data
			break;
			
		printf("2.receved data size:%ld\n",actual);
		
	}
	printf("3.receved data size:%ld\n",actual);
